...
const getTotalCount = (cart:{[id:string]:number}):number => {
    const total_array = Object.values(cart);
    const total = total_array.reduce((accumulator:any, currentValue:any) => accumulator + currentValue,0) as number;
    return total;
}
        ...
        <div className="sticky z-10 top-0 h-16 border-b bg-white lg:py-2.5">
            <div className="px-6 flex items-center justify-between space-x-4">
                ...
                <Link href='/dashboard/cart' className="p-2 flex items-center justify-center h-10 rounded-xl border bg-gray-100 focus:bg-gray-100 active:bg-gray-200">
                    {totalProducts > 0 && <span className='text-sm mr-2 font-bold'>{totalProducts}</span>}
